  ...
  <div>
    ...
    <div v-if="!isEmpty" style="position: relative">
      <div style="margin-bottom: 16px">
        <a-button type="dashed" @click="showCreateMadal">
          新建设备
        </a-button>
      ...
      >
        <span slot="option" slot-scope="text, record">
          ...
          <a @click="showAlterMadal(record.id,record.name,record.code,record.description,record.create_time)">更 改 </a>
        </span>
      </a-table>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleSubmitAlter(){
      this.axios.post('/api/alterDevice',{
        id:this.alterId,
    ...
    handleSubmitCreate(){
      this.axios.post('/api/createDevice',{
        token:this.$store.state.user.token,
